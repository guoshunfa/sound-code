...
final class CodeHTML implements Constants {
   ...
   private final String codeToHTML(ByteSequence bytes, int method_number) throws IOException {
      ...
      case 200:
      case 201:
         i = bytes.getIndex() + bytes.readInt() - 1;
         buf.append("<A HREF=\"#code" + method_number + "@" + i + "\">" + i + "</A>");
      }
   ...
   private final void findGotos(ByteSequence bytes, Method method, Code code) throws IOException {
      ...
      if (code != null) {
         ...
         label74:
         for(default_offset = 0; default_offset < attributes.length; ++default_offset) {
            if (attributes[default_offset].getTag() == 5) {
               LocalVariable[] vars = ((LocalVariableTable)attributes[default_offset]).getLocalVariableTable();
               npairs = 0;
      ...
      for(int var14 = 0; bytes.available() > 0; ++var14) {
         ...
         case 171:
            ...
            for(npairs = 0; npairs < no_pad_bytes; ++npairs) {
               bytes.readByte();
            }
   ...
   private void writeMethod(Method method, int method_number) throws IOException {
      ...
      if (code != null) {
         ...
         for(int var20 = 0; stream.available() > 0; ++var20) {
