...
public class MethodGen extends FieldGenOrMethodGen {
               ...
               if (a instanceof LineNumberTable) {
                  ...
                  if (a instanceof LocalVariableTable) {
                     ...
                     for(k = 0; k < lv.length; ++k) {
                        l = lv[k];
                        start = this.il.findHandle(l.getStartPC());
                     ...
                     for(k = 0; k < lv.length; ++k) {
                        ...
                        this.addLocalVariableType(l.getName(), Type.getType(l.getSignature()), l.getIndex(), start, end);
                     }
                  } else {
   ...
   public LocalVariableTypeTable getLocalVariableTypeTable(ConstantPoolGen cp) {
      LocalVariableGen[] lg = this.getLocalVariableTypes();
      int size = lg.length;
      LocalVariable[] lv = new LocalVariable[size];
   ...
   public Type[] getArgumentTypes() {
      return (Type[])((Type[])this.arg_types.clone());
   }
   ...
   public String[] getArgumentNames() {
      return (String[])((String[])this.arg_names.clone());
   }
      ...
      if (this.il != null) {
         ...
         for(InstructionHandle ih = this.il.getStart(); ih != null; ih = ih.getNext()) {
            ...
            if (ins instanceof LocalVariableInstruction || ins instanceof RET || ins instanceof IINC) {
               int index = ((IndexedInstruction)ins).getIndex() + ((TypedInstruction)ins).getType(this.cp).getSize();
               if (index > max) {
                  max = index;
               }
            }
   ...
   public static int getMaxStack(ConstantPoolGen cp, InstructionList il, CodeExceptionGen[] et) {
      MethodGen.BranchStack branchTargets = new MethodGen.BranchStack();

      int stackDepth;
      for(stackDepth = 0; stackDepth < et.length; ++stackDepth) {
         InstructionHandle handler_pc = et[stackDepth].getHandlerPC();
         if (handler_pc != null) {
      ...
      while(ih != null) {
         ...
         if (ih == null) {
            MethodGen.BranchTarget bt = branchTargets.pop();
            if (bt != null) {
               ih = bt.target;
   ...
   static final class BranchStack {
      ...
      public MethodGen.BranchTarget pop() {
         if (!this.branchTargets.empty()) {
            MethodGen.BranchTarget bt = (MethodGen.BranchTarget)this.branchTargets.pop();
            return bt;
         } else {
            return null;
         }
