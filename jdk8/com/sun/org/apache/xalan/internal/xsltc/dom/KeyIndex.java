...
public class KeyIndex extends DTMAxisIteratorBase {
   ...
   public IntegerArray getDOMNodeById(String id) {
      ...
      if (this._enhancedDOM != null) {
         ...
         if (ident != -1) {
            ...
            Map<String, IntegerArray> index = (Map)this._rootToIndexMap.get(root);
            if (index == null) {
               index = new HashMap();
               this._rootToIndexMap.put(root, index);
            } else {
               nodes = (IntegerArray)((Map)index).get(id);
            }

            if (nodes == null) {
               nodes = new IntegerArray();
               ((Map)index).put(id, nodes);
            }
   ...
   public int containsID(int node, Object value) {
      ...
      Map<String, IntegerArray> index = (Map)this._rootToIndexMap.get(rootHandle);
   ...
   public int containsKey(int node, Object value) {
      int rootHandle = this._dom.getAxisIterator(19).setStartNode(node).next();
      Map<String, IntegerArray> index = (Map)this._rootToIndexMap.get(new Integer(rootHandle));
      if (index == null) {
         return 0;
      } else {
   ...
   public KeyIndex.KeyIndexIterator getKeyIndexIterator(String keyValue, boolean isKeyCall) {
      return new KeyIndex.KeyIndexIterator(keyValue, isKeyCall);
   }
   ...
   public class KeyIndexIterator extends MultiValuedNodeHeapIterator {
      ...
      protected IntegerArray lookupNodes(int root, String keyValue) {
         ...
         Map<String, IntegerArray> index = (Map)KeyIndex.this._rootToIndexMap.get(root);
            ...
            for(int keyValueNode = keyValues.next(); keyValueNode != -1; keyValueNode = keyValues.next()) {
               ...
               if (nodes != null) {
                  if (!foundNodes) {
                     ...
                     if (this._nodes != null) {
                        this.addHeapNode(new KeyIndex.KeyIndexIterator.KeyIndexHeapNode(this._nodes));
                        this._nodes = null;
                     }

                     this.addHeapNode(new KeyIndex.KeyIndexIterator.KeyIndexHeapNode(nodes));
                  }
               }
            }
      ...
      protected class KeyIndexHeapNode extends MultiValuedNodeHeapIterator.HeapNode {
         ...
         public MultiValuedNodeHeapIterator.HeapNode cloneHeapNode() {
            KeyIndex.KeyIndexIterator.KeyIndexHeapNode clone = (KeyIndex.KeyIndexIterator.KeyIndexHeapNode)super.cloneHeapNode();
