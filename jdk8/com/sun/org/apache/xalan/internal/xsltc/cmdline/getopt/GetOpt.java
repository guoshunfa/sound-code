...
public class GetOpt {
   ...
   public GetOpt(String[] args, String optString) {
      ...
      for(i = 0; i < args.length; ++i) {
         ...
         if (token.startsWith("-") && tokenLength == 2) {
            this.theOptions.add(new GetOpt.Option(token.charAt(1)));
         } else {
            int indexoflast;
            if (token.startsWith("-") && tokenLength > 2) {
               for(indexoflast = 1; indexoflast < tokenLength; ++indexoflast) {
                  this.theOptions.add(new GetOpt.Option(token.charAt(indexoflast)));
               }
               ...
               GetOpt.Option op = (GetOpt.Option)this.theOptions.get(indexoflast);
   ...
   public void printOptions() {
      for(ListIterator it = this.theOptions.listIterator(); it.hasNext(); System.out.println()) {
         GetOpt.Option opt = (GetOpt.Option)it.next();
   ...
   public int getNextOption() throws IllegalArgumentException, MissingOptArgException {
      ...
      if (this.theOptionsIterator.hasNext()) {
         this.theCurrentOption = (GetOpt.Option)this.theOptionsIterator.next();
