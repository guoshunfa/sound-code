...
final class Whitespace extends TopLevelElement {
   ...
   private static Whitespace.WhitespaceRule findContradictingRule(Vector rules, Whitespace.WhitespaceRule rule) {
      for(int i = 0; i < rules.size(); ++i) {
         Whitespace.WhitespaceRule currentRule = (Whitespace.WhitespaceRule)rules.elementAt(i);
         if (currentRule == rule) {
            return null;
         }
   ...
   private static int prioritizeRules(Vector rules) {
      ...
      Whitespace.WhitespaceRule currentRule;
      int idx;
      for(idx = 0; idx < rules.size(); ++idx) {
         currentRule = (Whitespace.WhitespaceRule)rules.elementAt(idx);
      ...
      if (!strip) {
         ...
         while(true) {
            ...
            do {
               currentRule = (Whitespace.WhitespaceRule)rules.lastElement();
   ...
   private static void compilePredicate(Vector rules, int defaultAction, ClassGenerator classGen) {
      ...
      MethodGenerator stripSpace = new MethodGenerator(17, Type.BOOLEAN, new Type[]{Util.getJCRefType("Lcom/sun/org/apache/xalan/internal/xsltc/DOM;"), Type.INT, Type.INT}, new String[]{"dom", "node", "type"}, "stripSpace", classGen.getClassName(), il, cpg);
      ...
      for(int i = 0; i < rules.size(); ++i) {
         Whitespace.WhitespaceRule rule = (Whitespace.WhitespaceRule)rules.elementAt(i);
         ...
         if (rule.getStrength() == 2) {
            il.append((com.sun.org.apache.bcel.internal.generic.Instruction)(new ALOAD(paramDom)));
            il.append((com.sun.org.apache.bcel.internal.generic.Instruction)(new ILOAD(paramCurrent)));
            il.append((com.sun.org.apache.bcel.internal.generic.Instruction)(new INVOKEINTERFACE(gns, 2)));
            il.append((CompoundInstruction)(new PUSH(cpg, rule.getNamespace())));
            il.append((com.sun.org.apache.bcel.internal.generic.Instruction)(new INVOKEVIRTUAL(strcmp)));
            il.append(ICONST_0);
            if (rule.getAction() == 1) {
            ...
            il.append((com.sun.org.apache.bcel.internal.generic.Instruction)(new ILOAD(paramType)));
   ...
   private static int partition(Vector rules, int p, int r) {
      Whitespace.WhitespaceRule x = (Whitespace.WhitespaceRule)rules.elementAt(p + r >>> 1);
      int i = p - 1;
      int j = r + 1;
      ...
      while(true) {
         while(true) {
            --j;
            if (x.compareTo((Whitespace.WhitespaceRule)rules.elementAt(j)) >= 0) {
               do {
                  ++i;
               } while(x.compareTo((Whitespace.WhitespaceRule)rules.elementAt(i)) > 0);

               if (i >= j) {
                  return j;
               }

               Whitespace.WhitespaceRule tmp = (Whitespace.WhitespaceRule)rules.elementAt(i);
