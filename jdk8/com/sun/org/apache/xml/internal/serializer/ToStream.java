...
public abstract class ToStream extends SerializerBase {
   ...
   public OutputStream getOutputStream() {
      if (this.m_writer instanceof WriterToUTF8Buffered) {
         ...
         return this.m_writer instanceof WriterToASCI ? ((WriterToASCI)this.m_writer).getOutputStream() : null;
      }
   }
   ...
   public void elementDecl(String name, String model) throws SAXException {
      if (!this.m_inExternalDTD) {
         try {
            ...
            writer.write(model);
            writer.write(62);
            writer.write((char[])this.m_lineSep, 0, this.m_lineSepLen);
   ...
   protected final void outputLineSep() throws IOException {
      this.m_writer.write((char[])this.m_lineSep, 0, this.m_lineSepLen);
   }
   ...
   public void comment(char[] ch, int start, int length) throws SAXException {
      if (!this.m_inEntityRef) {
         ...
         try {
            ...
            Writer writer = this.m_writer;
            writer.write("<!--");
            int remainingChars = start;

            while(true) {
               if (remainingChars >= limit) {
                  if (length > 0) {
