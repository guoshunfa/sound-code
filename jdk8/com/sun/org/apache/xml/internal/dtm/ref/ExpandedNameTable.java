...
public class ExpandedNameTable {
   ...
   public ExpandedNameTable() {
      ...
      this.m_table = new ExpandedNameTable.HashEntry[this.m_capacity];
   ...
   private void initExtendedTypes() {
      ...
      for(int i = 0; i < 14; ++i) {
         ...
         this.m_table[i] = new ExpandedNameTable.HashEntry(m_defaultExtendedTypes[i], i, i, (ExpandedNameTable.HashEntry)null);
      }
   ...
   public int getExpandedTypeID(String namespace, String localName, int type, boolean searchOnly) {
      ...
      for(ExpandedNameTable.HashEntry e = this.m_table[index]; e != null; e = e.next) {
      ...
      if (searchOnly) {
         ...
         ExpandedNameTable.HashEntry entry = new ExpandedNameTable.HashEntry(newET, this.m_nextType, hash, this.m_table[index]);
   ...
   private void rehash() {
      ...
      ExpandedNameTable.HashEntry[] oldTable = this.m_table;
      int newCapacity = 2 * oldCapacity + 1;
      this.m_capacity = newCapacity;
      this.m_threshold = (int)((float)newCapacity * m_loadFactor);
      this.m_table = new ExpandedNameTable.HashEntry[newCapacity];

      ExpandedNameTable.HashEntry e;
      int newIndex;
      for(int i = oldCapacity - 1; i >= 0; --i) {
         for(ExpandedNameTable.HashEntry old = oldTable[i]; old != null; this.m_table[newIndex] = e) {
            e = old;
            old = old.next;
   ...
   private static final class HashEntry {
      ...
      protected HashEntry(ExtendedType key, int value, int hash, ExpandedNameTable.HashEntry next) {
         this.key = key;
         this.value = value;
         this.hash = hash;
         this.next = next;
