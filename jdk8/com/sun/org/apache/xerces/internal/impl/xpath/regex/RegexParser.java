...
class RegexParser {
   ...
   Token processCondition() throws ParseException {
      if (this.offset + 1 >= this.regexlen) {
         ...
         if ('1' <= ch && ch <= '9') {
            ...
            this.references.addElement(new RegexParser.ReferencePosition(finalRefno, this.offset));
            ++this.offset;
   ...
   Token processBackreference() throws ParseException {
      ...
      if (this.parennumber <= refnum) {
         ...
         this.references.addElement(new RegexParser.ReferencePosition(finalRefnum, this.offset - 2));
         this.next();
         return tok;
      }
   }
   ...
   protected RangeToken parseCharacterClass(boolean useNrange) throws ParseException {
      ...
      int type;
      for(boolean firstloop = true; (type = this.read()) != 1 && (type != 0 || this.chardata != 93 || firstloop); firstloop = false) {
         int c = this.chardata;
         boolean end = false;
         int rangeend;
         if (type == 10) {
