...
public class XMLDTDValidator implements XMLComponent, XMLDocumentFilter, XMLDTDValidatorFilter, RevalidationHandler {
   ...
   public void doctypeDecl(String rootElement, String publicId, String systemId, Augmentations augs) throws XNIException {
      ...
      this.fRootElement.setValues((String)null, rootElement, rootElement, (String)null);
      String eid = null;
   ...
   protected void addDTDDefaultAttrsAndValidate(QName elementName, int elementIndex, XMLAttributes attributes) throws XNIException {
      if (elementIndex != -1 && this.fDTDGrammar != null) {
         ...
         for(int attlistIndex = this.fDTDGrammar.getFirstAttributeDeclIndex(elementIndex); attlistIndex != -1; attlistIndex = this.fDTDGrammar.getNextAttributeDeclIndex(attlistIndex)) {
            ...
            attrRawName = this.fTempAttDecl.name.rawname;
            ...
            changedByNormalization = attDefaultType == 2;
            boolean cdata = attType == XMLSymbols.fCDATASymbol;
            int index;
            if (!cdata || changedByNormalization || attValue != null) {
               index = attributes.getLength();

               for(int i = 0; i < index; ++i) {
                  if (attributes.getQName(i) == attrRawName) {
                     specified = true;
                     break;
                  }
               }
            ...
            if (!specified) {
               ...
               if (changedByNormalization) {
                  if (this.fPerformValidation) {
                     args = new Object[]{elementName.localpart, attrRawName};
                  ...
                  if (this.fNamespaces) {
                     index = attrRawName.indexOf(58);
                     if (index != -1) {
                        attPrefix = attrRawName.substring(0, index);
                        attPrefix = this.fSymbolTable.addSymbol(attPrefix);
                        attLocalpart = attrRawName.substring(index + 1);
                        attLocalpart = this.fSymbolTable.addSymbol(attLocalpart);
                     }
                  }

                  this.fTempQName.setValues(attPrefix, attLocalpart, attrRawName, this.fTempAttDecl.name.uri);
         ...
         for(int i = 0; i < attrCount; ++i) {
            attrRawName = attributes.getQName(i);
            boolean declared = false;
            if (this.fPerformValidation && this.fGrammarBucket.getStandalone()) {
               String nonNormalizedValue = attributes.getNonNormalizedValue(i);
               if (nonNormalizedValue != null) {
                  attValue = this.getExternalEntityRefInAttrValue(nonNormalizedValue);
                  if (attValue != null) {
                     this.fErrorReporter.reportError("http://www.w3.org/TR/1998/REC-xml-19980210", "MSG_REFERENCE_TO_EXTERNALLY_DECLARED_ENTITY_WHEN_STANDALONE", new Object[]{attValue}, (short)1);
                  }
               }
            }

            int attDefIndex = true;
   ...
   protected void validateDTDattribute(QName element, String attValue, XMLAttributeDecl attributeDecl) throws XNIException {
      ...
      case 1:
         found = attributeDecl.simpleType.list;
      ...
      case 3:
         ...
         } catch (InvalidDatatypeValueException var8) {
            this.fErrorReporter.reportError("http://www.w3.org/TR/1998/REC-xml-19980210", var8.getKey(), var8.getArgs(), (short)1);
         }
         break;
      case 4:
         found = attributeDecl.simpleType.list;
         ...
         try {
            ...
            if (found) {
               ...
               this.fErrorReporter.reportError("http://www.w3.org/TR/1998/REC-xml-19980210", var11.getKey(), var11.getArgs(), (short)1);
            }
         }
         break;
      case 5:
