...
class MethodWriter implements MethodVisitor {
   ...
   MethodWriter(ClassWriter cw, int access, String name, String desc, String signature, String[] exceptions, boolean computeMaxs, boolean computeFrames) {
      ...
      if (computeMaxs || computeFrames) {
         ...
         size = getArgumentsAndReturnSizes(this.descriptor) >> 2;
         if ((access & 8) != 0) {
            --size;
         }
         ...
         if (type == -1) {
            ...
            for(i = 0; i < nLocal; ++i) {
               if (local[i] instanceof String) {
                  this.frame[this.frameIndex++] = 24117248 | this.cw.addType((String)local[i]);
               } else if (local[i] instanceof Integer) {
                  this.frame[this.frameIndex++] = (Integer)local[i];
               } else {
                  this.frame[this.frameIndex++] = 25165824 | this.cw.addUninitializedType("", ((Label)local[i]).position);
               }
            }

            for(i = 0; i < nStack; ++i) {
               if (stack[i] instanceof String) {
                  this.frame[this.frameIndex++] = 24117248 | this.cw.addType((String)stack[i]);
               } else if (stack[i] instanceof Integer) {
                  this.frame[this.frameIndex++] = (Integer)stack[i];
               } else {
                  this.frame[this.frameIndex++] = 25165824 | this.cw.addUninitializedType("", ((Label)stack[i]).position);
               }
            }
            ...
            switch(type) {
            case 0:
               this.stackMap.putByte(255).putShort(i).putShort(nLocal);
               ...
               break;
            case 1:
               this.stackMap.putByte(251 + nLocal).putShort(i);
               i = 0;
   ...
   public void visitTableSwitchInsn(int min, int max, Label dflt, Label[] labels) {
      ...
      ByteVector var10000 = this.code;
      var10000.length += (4 - this.code.length % 4) % 4;
         ...
         for(l = this.labels; l != null; l = l.successor) {
            ...
            if ((l.status & 64) == 0) {
               ...
               if (end >= start) {
                  ...
                  this.frame[this.frameIndex++] = 24117248 | this.cw.addType("java/lang/Throwable");
                  this.endFrame();
               }
            }
         }
         ...
         if (this.subroutines > 0) {
            ...
            for(stack = this.labels; stack != null; stack = stack.successor) {
               if ((stack.status & 128) != 0) {
                  ...
                  subroutine = stack.successors.next.successor;
                  subroutine.visitSubroutine(stack, 0L, this.subroutines);
               }
            }
         }
